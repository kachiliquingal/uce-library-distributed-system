...
export class KafkaProducer {
  ...
  public async connect(): Promise<void> {
    try {
      ...
      logger.info(`[Kafka] Auth Service connected to Kafka at ${process.env.KAFKA_BROKERS || 'localhost:9092'}`);
    } catch (error) {
      logger.error('[Kafka] Error connecting Auth Service to Kafka:', error);
    }
  }
