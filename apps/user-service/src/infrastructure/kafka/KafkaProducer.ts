...
export class KafkaProducer {
  ...
  public async connect(): Promise<void> {
    try {
      ...
      console.log(`[Kafka] User Service connected to Kafka at ${process.env.KAFKA_BROKERS || 'localhost:9092'}`);
    } catch (error) {
      console.error('[Kafka] Error connecting User Service to Kafka:', error);
    }
  }
