...
export class Neo4jUserRepository implements UserRepository {
  ...
  private mapUser(record: any): User {
    ...
    return {
      ...
      roles: rolesNode.map((r: any) => ({
        ...
        permissions: [] // permissions would be loaded similarly if needed
      }))
    };
  }
  ...
  async updateUser(id: string, data: Partial<Omit<User, "id" | "createdAt" | "updatedAt" | "roles">>): Promise<User | null> {
    ...
    try {
      const now = new Date().toISOString();
      const updates = Object.keys(data).map(k => `u.${k} = $${k}`).join(", ");
      
      if (!updates) return this.getUserById(id);

      const result = await session.run(
        `
        MATCH (u:User {id: $id})
        SET ${updates}, u.updatedAt = $now
        WITH u
        OPTIONAL MATCH (u)-[:HAS_ROLE]->(r:Role)
        RETURN u, collect(r) as roles
        `,
  ...
  async removeRoleFromUser(userId: string, roleName: string): Promise<void> {
    ...
    try {
      await session.run(
        `
        MATCH (u:User {id: $userId})-[rel:HAS_ROLE]->(r:Role {name: $roleName})
        DELETE rel
        `,
        { userId, roleName }
      );
